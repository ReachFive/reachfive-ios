...
class CredentialManager: NSObject {
    ...
    /// continuation is in place before the delegate can fire.
    ///
    ...
    /// Assembles the modal sign-in request batch, in the requested order — the system presents the
    /// credentials in that order. Pure function: no network, no class state.
    ///
    /// A type this function cannot prepare (no Apple provider configured, passkey request already failed)
    /// is dropped from the batch rather than blocking sign-in when another type can still succeed; the
    /// first dropped reason only surfaces if nothing survives.
    ///
    /// - Parameter passkey: the assertion request the caller already built, or the error it hit while
    ///   building it; nil when `.Passkey` was not requested.
    ///
    /// Internal for testability.
...
extension CredentialManager: ASAuthorizationControllerDelegate {
    ...
    /// Removes the context as soon as the callback fires: guarantees a continuation is resumed exactly
