...
extension LoginWKWebview: WKNavigationDelegate {
    public func webView(_ webView: WKWebView, decidePolicyFor navigationAction: WKNavigationAction) async -> WKNavigationActionPolicy {
        guard let reachfive,
              ...
              // Both sides folded: a scheme is case-insensitive (RFC 3986) but Foundation does not normalise
              // `URL.scheme`, so it comes back with the case the redirect_uri was registered with — and the
              // default scheme is derived from the clientId, which is usually mixed case.
