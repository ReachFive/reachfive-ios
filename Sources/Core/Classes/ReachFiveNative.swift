...
extension ReachFive {
    ...
    public func login(withRequest request: NativeLoginRequest, usingModalAuthorizationFor requestTypes: [ModalAuthorization], display mode: Mode, upgradingToPasskey passkeyFriendlyName: String? = nil) async throws -> LoginFlow {
        ...
        return try await credentialManager.login(withRequest: try resolve(request), usingModalAuthorizationFor: requestTypes, display: mode, appleProvider: appleProvider, upgradingToPasskey: passkeyFriendlyName, reachFive: self)
    }
    ...
    public func upgradeToPasskey(withRequest request: NewPasskeyRequest, authToken: AuthToken) async throws -> Bool {
        ...
        return try await credentialManager.upgradeToPasskey(withRequest: request, originWebAuthn: originWebAuthn(overriddenBy: request.originWebAuthn), authToken: authToken, reachFive: self)
    }
