...
<3> Optional; defaults to `.customScheme`, which also covers providers that detour through a third-party app.
Use `.universalLink` only when the login must come back on a universal link intercepted in the sheet — see xref:providerCreator.adoc#universal-link-web-providers[] and xref:guides/apple-app-site-association.adoc#webcredentials[].
