...
For web-based logout, the `post_logout_redirect_uri` must be configured in the {cc}'s SSO settings as an *Allowed logout callback URL*. 
Failure to configure this will result in a “URI redirect not allowed” error.
****

include::docs:ROOT:partial$tokens/r_revoke-refresh-token-note.adoc[]
...
include::partial$params.adoc[tags="webSessionLogout,revoke"]
