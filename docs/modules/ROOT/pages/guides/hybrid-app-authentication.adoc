= Authenticating in a hybrid app
...
Before we dive in, it's important you understand the key terms so we're all on the same page.
...
Using the native approach, users authenticates in the native iOS application and navigate through the mobile app whether it is through other native screens or WK webviews.
They can leverage their native app session on other mobile apps as long as it is synchronised with their Apple account and permitted by the integrator (that's you).
