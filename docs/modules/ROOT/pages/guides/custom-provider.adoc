...
The actual `Provider` calls into the third-party native SDK, then exchanges its result for a {company} `AuthToken`.
...
|Your provider's callback arrives through a custom URL scheme (`myapp://...`).
...
Declare `applinks:<host>` and the matching `applinks` section in `apple-app-site-association` (see xref:guides/apple-app-site-association.adoc#applinks[]).
