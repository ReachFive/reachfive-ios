...
* `action`: A string describing the action for the MFA flow.
...
* `presenting`: Where the credential provider presents its UI, built with `Presentation(from: self)` from a view controller attached to a window (`viewDidAppear` or a user interaction, not `viewDidLoad`).
* `originWebAuthn`: The origin of the webauthn call.
Defaults to the `originWebAuthn` of your `SdkConfig`, or to `\https://$\{domain\}`.
...
TIP: For more, see link:https://developer.apple.com/documentation/authenticationservices/aswebauthenticationsession/3237232-presentationcontextprovider[Apple docs^].
...
|Controls the behavior of the `ASWebAuthenticationSession` callback and its `redirect_uri`. Defaults to `.customScheme`.
...
* `.universalLink(URL)` (iOS 17.4+): the universal-link redirect is intercepted directly inside the authentication sheet.
Requires the `webcredentials:` Associated Domain.
See xref:guides/apple-app-site-association.adoc#webcredentials[].

TIP: See the xref:providerCreator.adoc[WebProvider] entry for a ready-made provider using this mechanism.
// end::webSessionMode[]

// tag::loginUrlFragment[]
|[[loginUrlFragment,loginUrlFragment]] loginUrlFragment `[String: String]?`
|Key/value pairs propagated in the *fragment* of the link:{site-url}/openapi/identity.html#operation/authorizeUser[`/oauth/authorize` URL] (e.g., `#key=value&key2=value2`).

Intended for the token-orchestration flow: link:{site-url}/openapi/identity.html#operation/authorizeUser[`/oauth/authorize`] redirects to the client's Login URL.
A fragment, unlike a query parameter, survives that redirect; the browser reapplies it onto the redirect target.
The Login URL can then read it via `window.location.hash` to customize itself (logo, colors) per calling channel.
...
| *optional* A captcha token your application obtained, forwarded as is to {company}.

Needed as soon as the client's configuration enables a captcha on the endpoint this method calls, and ignored otherwise.
ifeval::["{docname}" == "loginWithPassword"]
In that configuration, this endpoint is named `password_login`.
endif::[]
ifeval::["{docname}" == "signup"]
In that configuration, this endpoint is named `signup_token`.
endif::[]
ifeval::["{docname}" == "requestPasswordReset"]
In that configuration, this endpoint is named `forgot_password`.
endif::[]
ifeval::["{docname}" == "requestAccountRecovery"]
In that configuration, this endpoint is named `forgot_password`.
endif::[]
ifeval::["{docname}" == "startPasswordless"]
In that configuration, this endpoint is named `passwordless`.
endif::[]
ifeval::["{docname}" == "updateEmail"]
In that configuration, this endpoint is named `update_email`.
endif::[]

The SDK never produces the token: obtaining one is your application's job.
A token is single-use and short-lived, and the action it was minted with is sealed inside it, so there is no action parameter to pass here.
See xref:guides/captcha.adoc[] for how to obtain one and for the pitfalls.
// end::captcha[]
...
// tag::webSessionLogout[]
|[[webSessionLogout,webSessionLogout]] webSessionLogout `WebSessionLogoutRequest?`
| _optional_ The web session logout request.

Defaults to `nil`.

If provided, triggers a <<WebSessionLogoutRequest, web-based logout>> by opening a web authentication session to the server's logout endpoint.

The URL must be defined in your {cc} settings at menu:Security[SSO > Allowed Callback URLs].
// end::webSessionLogout[]

// tag::WebSessionLogoutRequest[]
|[[WebSessionLogoutRequest,WebSessionLogoutRequest]] WebSessionLogoutRequest
...
* _required_ `presentationContextProvider`: A delegate that provides a display context whereby the system is able to present an authentication session to the user.
* _optional_ `origin`: The origin of the call.

// end::WebSessionLogoutRequest[]

// tag::revoke[]
|[[revoke,revoke]] revoke `AuthToken?`
| _optional_ The authentication token to revoke, invalidating the user's session.
Use `nil` to skip token revocation.

Defaults to `nil`.

NOTE: The `logout` method invalidates any tokens found here.
