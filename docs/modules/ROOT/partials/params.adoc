...
If provided, triggers a <<WebSessionLogoutRequest, web-based logout>> by opening a web authentication session to the server's logout endpoint (e.g., `/logout`). 
This clears browser cookies and redirects to a configured `post_logout_redirect_uri`.
