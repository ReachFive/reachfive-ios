...
* appSwitch `FBSDKLoginKit.AppSwitch`: whether a classic login may open the native Facebook app rather than an in-app browser.
Defaults to `.enabled`; pass `.disabled` to force the browser flow.
It has no effect on a limited login, and none unless your `Info.plist` declares the `fbauth2` scheme.
See xref:index.adoc#facebook-fast-app-switch[Fast App Switch].
