...
** A web login was dropped because another one was already in progress. See xref:webviewLogin.adoc[].
** The app is not associated with the host of an xref:providerCreator.adoc#universal-link-web-providers[`WebProvider` `.universalLink`] callback. See xref:guides/apple-app-site-association.adoc#webcredentials[].
--
