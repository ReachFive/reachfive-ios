...
- `presenting` `Presentation`: Where the credential provider presents its UI, built with `Presentation(from: self)` from a view controller attached to a window (`viewDidAppear` or a user interaction, not `viewDidLoad`).
...
The user cancelled the request, no credential was available in the keychain, a web login was dropped because another one was already in progress (see xref:webviewLogin.adoc[]), or the app is not associated with the host of an `.https` callback (see xref:guides/apple-app-site-association.adoc#webcredentials[]).
...
// tag::AchievedLoginSignup[]
=== AchievedLogin

Returned when a signup completes successfully and the user is immediately logged in.
