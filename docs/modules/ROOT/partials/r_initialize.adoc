...
<2> Your {company} domain, as a bare host such as `integ-sandbox.reach5.dev` — no `https://` prefix, no port and no trailing slash.
...
For more details, see xref:application/applicationDidFinishLaunchingWithOptions.adoc[].
<3> Forwards universal links to {company}.
Required by WeChat and by custom providers whose callback is a universal link; not by `WebProvider` `.universalLink` or by a login that detours through a third-party app.
For more details, see xref:application/applicationContinueUserActivity.adoc[].
<4> Forwards custom-scheme callbacks used by passwordless, MFA, account recovery, and web logins that return through the default browser.
