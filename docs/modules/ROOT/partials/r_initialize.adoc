...
Required by WeChat and by custom providers whose callback is a universal link; not by `WebProvider` `.universalLink`.
For more details, see xref:application/applicationContinueUserActivity.adoc[].
<4> Forwards custom-scheme callbacks used by passwordless, MFA, account recovery, and provider native apps.
