...
- A network interception layer that answers `willPerformHTTPRedirection` for the SDK's session — SSL pinning, an APM agent, a `URLProtocol` — no longer breaks the login. The callback is now read from the `Location` header of the redirection that ends the task, which is what refusing to follow it leaves behind whoever refused: the SDK's own delegate, or the interception layer answering in its place. When the interception lets `URLSession` follow the redirection instead, loading the app's custom scheme fails with `unsupportedURL` and the callback is recovered from that error's failing URL. A redirection leading elsewhere — an interception rewriting the target to a block page, or one stripped of its `Location` — now names where it actually leads. A certificate the pinning refuses stays an untouched `URLError`.
- A request whose task ends with neither a response nor an error no longer takes the app down. Every call went through `URLSession.data(for:)`, whose back-deployed form — the only one available at this SDK's iOS 13 floor — force-unwraps the response and traps on that outcome, which is exactly what an interception layer standing in for `URLSession` can produce. The SDK now reports it as a `TechnicalError` naming the interception.
...
### New features
- Support the b.connect provider: handle the login flow through an external app and add its `cico` claim to OpenIdUser.
