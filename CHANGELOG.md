...
### Bug fixes
- `loadLoginWebview` no longer hangs forever when the custom scheme contains uppercase letters, which is the default case since it is derived from the clientId. The callback comparison lowercased only one side, so the webview never recognised its own callback.
